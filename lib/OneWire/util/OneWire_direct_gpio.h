...
#if defined(__AVR_ATmega4809__) || defined(DXCORE)
// Modern-AVR PORT struct (megaAVR-0/1-series, AVR Dx-series): portInputRegister() returns
// PORTx.IN; PORTx.DIR sits 8 bytes before it, PORTx.OUT sits 4 bytes before it. Upstream only
// special-cased __AVR_ATmega4809__; DXCORE (defined by DxCore for all AVR Dx-series parts,
// including the AVR128DB48 this project targets) needs the same offsets or these macros
// silently fall through to the classic-AVR PINx/DDRx/PORTx layout below and write to the
// wrong registers entirely — no 1-Wire bus activity, pin reads as permanently idle.
